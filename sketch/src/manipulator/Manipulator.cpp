...
bool Manipulator::recursivelyFindArmsRotationToMoveToDestionation(const uint8_t& armId, const Vector2& destinationPointInTwoDimensionalSpace, const float& allowedZoneAroundDest, Vector2 armBasePos, ArduinoList<float>& armsAngles, bool upstreamPassage) {
    ArduinoList<Vector2> possibleMovePoints = ArduinoVectorMath::findCirclesIntersectionPoints(destinationPointInTwoDimensionalSpace, allowedZoneAroundDest, armBasePos, this->manipulatorArms[armId]->getLength());
    Serial.print("  arm id: ");
    ...
    Serial.print("count of possible move points: ");
...
Vector2 Manipulator::getVectorOfArmUsingListOfAnglesInTwoDimensionalSpace(uint8_t armId, const ArduinoList<float>& listOfArmsAngles) {
    float armAngle = 0;
    for (uint8_t prevArmId = 0; prevArmId <= armId; prevArmId++) {
        armAngle += listOfArmsAngles[prevArmId] - this->manipulatorArms[prevArmId]->getArmServo()->getAngleShift();
    }
    return ArduinoVectorMath::makeVectorFromLengthAndAngle(this->manipulatorArms[armId]->getLength(), armAngle);
}
...
void Manipulator::setCurrentPosition(const Vector3& newPosition) {
    Serial.print("Arms total length: ");
    Serial.println(this->armsTotalLength);
    Serial.print("Distance to new pos: ");
    Serial.println(newPosition.getLength());
    if (newPosition.getLength() > this->armsTotalLength) {
        newPosition = newPosition.getNormalized() * this->armsTotalLength;
        Serial.print("Normalized distance to new pos: ");
        Serial.println(newPosition.getLength());
    }
