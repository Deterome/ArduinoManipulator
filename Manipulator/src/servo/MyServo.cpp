...
MyServo::MyServo(uint8_t pinOnDemultiplexer, float angleIncreaseCoef, AngleOverloadAction angleOverloadAction, int16_t angleShift, int16_t lowerAngleLimit, int16_t upperAngleLimit) : MyServo(pinOnDemultiplexer, angleIncreaseCoef, angleOverloadAction, angleShift, lowerAngleLimit, upperAngleLimit, DEFAULT_MIN_SERVO180_MICROS, DEFAULT_MAX_SERVO180_MICROS) {}

MyServo::MyServo(uint8_t pinOnDemultiplexer, float angleIncreaseCoef, AngleOverloadAction angleOverloadAction, int16_t angleShift, int16_t lowerAngleLimit, int16_t upperAngleLimit, uint16_t minWidthOfPulse, uint16_t maxWidthOfPulse) {
...
void MyServo::setAngleShift(int16_t angleShift) {
    this->lowerAngleLimit -= angleShift - this->angleShift; 
    this->upperAngleLimit -= angleShift - this->angleShift; 
...
void MyServo::setServoAngle(float newServoAngle) {
    ...
    if (this->servoAngle != newServoAngle) {
        ...
        this->servoAngleInMicros = this->angleToMicros((newServoAngle + this->angleShift)/this->angleIncreaseCoef);
    }
}
